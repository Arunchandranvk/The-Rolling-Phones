...
class StoreSearch(TemplateView):
    template_name="s_search.html"
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        query=self.request.GET.get('query')
        if query:
               context["data"]=Product.objects.filter(Q(productname__icontains=query) | Q(model__icontains=query) | Q(description__icontains=query)).order_by("-datetime")
